...
class BaseLearner(object):
    ...
    def _reduce_exemplar(self, data_manager, m_label, m_unlabel):
        ...
        for class_idx in range(self._known_classes):
            mask_sp = np.where(dummy_targets == class_idx)[0]
            dd, dt = dummy_data[mask_sp][:m_label], dummy_targets[mask_sp][:m_label]
    ...
    def _construct_exemplar_111111(self, data_manager, m_label, m_unlabel):
        logging.info("Constructing supervised exemplars...({} per classes)".format(m_label))
        logging.info("Constructing unsupervised exemplars...({} per classes)".format(m_unlabel))
        for class_idx in range(self._known_classes, self._total_classes):
            ...
            label_idx = np.where(idx_dataset.lab_index_task==1)[0]
            ...
            ## Select labeled data
            if m_label: # and m_label<label_idx.shape[0]
                vectors_label = vectors[label_idx]
                data_label = data[label_idx]
                class_mean_supervise = np.mean(vectors_label, axis=0)
                for k in range(1, m_label + 1):
                    if len(vectors_label)<1:
                        break
                    S = np.sum(
                        exemplar_vectors, axis=0
                    )  # [feature_dim] sum of selected exemplars vectors
                    mu_p = (vectors_label + S) / k  # [n, feature_dim] sum to all vectors
                    i = np.argmin(np.sqrt(np.sum((class_mean_supervise - mu_p) ** 2, axis=1)))
                    selected_exemplars.append(
                        np.array(data[i])
                    )  # New object to avoid passing by inference
                    exemplar_vectors.append(
                        np.array(vectors_label[i])
                    )  # New object to avoid passing by inference
                    ...
                    vectors_label = np.delete(
                        vectors_label, i, axis=0
                    )  # Remove it to avoid duplicative selection
                    data_label = np.delete(
                        data_label, i, axis=0
                    )  # Remove it to avoid duplicative selection
            '''
            elif m_label and m_label>=label_idx.shape[0]:
                vectors_super = vectors[label_idx]
                data_super = data[label_idx]
                class_mean_supervise = np.mean(vectors_super, axis=0)

                for k in range(label_idx.shape[0]):
                    ...
                    selected_exemplars.append(
                        np.array(data_super[k])
                    )  # New object to avoid passing by inference
                    selected_exemplars_lab_idx.append(np.array(1))
                    selected_exemplars_targets.append(np.array(class_idx))
                    selected_exemplars_pse_targets.append(np.ones_like(class_idx)*-100)
                    exemplar_vectors.append(
                        np.array(vectors_super[k])
                    )
            '''
            if m_unlabel and (class_idx) == self._total_classes-1: # 后者判断的前提是base的类别数量个数要大于等于incre的数量  and m_unlabel<(data.shape[0]-label_idx.shape[0])
                # load all data
                data, targets, idx_dataset = data_manager.get_dataset(
                ...
                m_unlabel = m_unlabel * (self._total_classes - self._known_classes)
                idx_loader = DataLoader(
                    idx_dataset, batch_size=batch_size, shuffle=False, num_workers=4
                )
                vectors, _, _, logits, pseulabel = self._extract_vectors_and_psedolabel(idx_loader)
                vectors = (vectors.T / (np.linalg.norm(vectors.T, axis=0) + EPSILON)).T

                unlabel_idx = np.where((idx_dataset.lab_index_task==0) * (logits.max(1)>self.oldpse_thre))[0]  ## pseudo labels is supposed to be bigger than threshold
                if len(unlabel_idx) < m_unlabel:
                    m_unlabel = len(unlabel_idx)
                vectors_unsuper = vectors[unlabel_idx]
                data_unsuper = data[unlabel_idx]
                #targets_unsuper = targets[unlabel_idx]
                ...
                pse_targets_unsuper = pseulabel[unlabel_idx]  #
                
                for k in range(1, m_unlabel + 1):
                    S = np.sum(
                        exemplar_vectors, axis=0
                    )  # [feature_dim] sum of selected exemplars vectors
            ...
            '''
            elif m_unlabel and (class_idx) == self._total_classes-1 and m_unlabel>=(data.shape[0]-label_idx.shape[0]):
                 # 加载所有的数据
                data, targets, idx_dataset = data_manager.get_dataset(
                np.arange(self._known_classes, self._total_classes),
                source="train", 
                ...
                unlabel_idx = np.where((idx_dataset.lab_index_task==0) * (logits.max(1)>self.oldpse_thre))[0]  
                if len(unlabel_idx) < m_unlabel:
                    m_unlabel = len(unlabel_idx)
                vectors_unsuper = vectors[unlabel_idx]
                data_unsuper = data[unlabel_idx]
                ...
                for k in range(m_unlabel):
                    
                    selected_exemplars.append(
                        np.array(data_unsuper[k])
                    )  # New object to avoid passing by inference
                    selected_exemplars_lab_idx.append(np.array(0))  # 
            ...
            # Exemplar mean of  labeled data
            idx_dataset, _ = data_manager.get_dataset(
                [],
                source="train",
                mode="test",
                appendent=(selected_exemplars[:m_label], exemplar_targets[:m_label], pse_exemplar_targets[:m_label], selected_exemplars_lab_idx[:m_label]),
            )
            idx_loader = DataLoader(
                idx_dataset, batch_size=batch_size, shuffle=False, num_workers=4
            )
    ...
    def _construct_exemplar(self, data_manager, m_super, m_unsuper):
        ...
        for class_idx in range(self._known_classes, self._total_classes): #self._total_classes-self._known_classes=self.increment
            ...
            '''
            elif m_super and m_super>=super_idx.shape[0]:
                vectors_super = vectors[super_idx]
                data_super = data[super_idx]
                class_mean_supervise = np.mean(vectors_super, axis=0)

                for k in range(super_idx.shape[0]):
                    if len(vectors_super)<1:
                        break
                    selected_exemplars.append(
                        np.array(data_super[k])
